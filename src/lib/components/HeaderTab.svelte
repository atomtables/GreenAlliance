...
{#if isDropdown}
    <button onclick={() => dropdownActive = !dropdownActive}
            class="font-bold text-gray-800 dark:text-gray-300 hover:text-inherit hover:bg-neutral-500/40 active:bg-neutral-400/40 {dropdownActive && '!bg-neutral-400/40 !text-inherit'} cursor-pointer transition-all {(url?.includes(href) || url?.includes(activeUrl) || href === '/' && url === href) && 'current-tab'} {!custom && (showOnAuth ? 'auth' : 'noauth')} {className}">
        <span class="block py-4 px-5 uppercase">{name}</span>
        {#if dropdownActive}
            <span class="absolute flex flex-col bg-gray-200 dark:bg-gray-800 text-left" transition:slide={{axis: 'y'}}>
                {#each elements as {name, url}}
                    <a class="uppercase py-4 px-5 font-bold text-gray-800 dark:text-gray-300 hover:text-inherit hover:bg-neutral-500/40 active:bg-neutral-400/40 cursor-pointer transition-all {url === '/account' && 'current-tab'} auth"
                       href={url} >
                        {name}
                    </a>
                {/each}
