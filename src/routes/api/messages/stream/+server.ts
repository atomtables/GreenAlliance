...
export const GET: RequestHandler = async ({ params, locals }) => {
    ...
    return produce(async ({ emit, lock }) => {
        ...
        if (!_clients[locals.user.id]) _clients[locals.user.id] = {};
        _clients[locals.user.id][sessionId] = emit;
        emit("session", sessionId);

        for (const userId in _clients) {
            if (userId === locals.user.id) continue; // don't send to self
            emit("presence", JSON.stringify({ userId: locals.user.id, status: "online" }));
        }
        ...
        for (const userId in _clients) {
            ...
            for (const sessionId in _clients[userId]) {
                _clients[userId][sessionId]("presence", JSON.stringify({ userId: locals.user.id, status: "online" }));
            }
        }
    }, {
        stop() {
            // unregister the user
            if (_clients?.[locals.user.id]) {
                delete _clients[locals.user.id][sessionId];
            }
            for (const userId in _clients) {
                if (userId === locals.user.id) continue; // don't send to self
                for (const sessionId in _clients[userId]) {
                    _clients[userId][sessionId]("presence", JSON.stringify({ userId: locals.user.id, status: "offline" }));
                }
            }
        }
    })
